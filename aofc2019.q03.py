...
def find_distance(wireA, wireB):
    coordsA = get_coords(wireA)
    coordsB = get_coords(wireB)
    crossABs = set(coordsA) & set(coordsB) - set([(0,0)])
    lowAB = -1
    for crossAB in crossABs:
        sumAB = abs(crossAB[0]) + abs(crossAB[1])
        if lowAB == -1 or sumAB < lowAB:
            lowAB = sumAB
    return lowAB

def get_steps(coords, end):
    steps = 0
    for coord in coords:
        if coord == end:
            break
...
def find_steps(wireA, wireB):
    coordsA = get_coords(wireA)
    coordsB = get_coords(wireB)
    crossABs = set(coordsA) & set(coordsB) - set([(0,0)])
    lowAB = -1
    for crossAB in crossABs:
        stepsA = get_steps(coordsA, crossAB)
        stepsB = get_steps(coordsB, crossAB)
        stepsAB = stepsA + stepsB
        if lowAB == -1 or stepsAB < lowAB:
            lowAB = stepsAB
    return lowAB

# EGs for 3 (a).
eg3wire1A = ['R8','U5','L5','D3']
eg3wire1B = ['U7','R6','D4','L4'] 
eg3wire2A = ['R75','D30','R83','U83','L12','D49','R71','U7','L72']
eg3wire2B = ['U62','R66','U55','R34','D71','R55','D58','R83']
eg3wire3A = ['R98','U47','R26','D63','R33','U87','L62','D20','R33','U53','R51']
eg3wire3B = ['U98','R91','D20','R16','D67','R40','U7','R15','U6','R7'] 
...
print (find_distance(eg3wire1A, eg3wire1B)) # distance 6
print (find_distance(eg3wire2A, eg3wire2B)) # distance 159
print (find_distance(eg3wire3A, eg3wire3B)) # distance 135
print ("3) answer part (a):", find_distance(wireA, wireB)) # distance 731
print ()

print (find_steps(eg3wire1A, eg3wire1B)) # 30  steps
print (find_steps(eg3wire2A, eg3wire2B)) # 610 steps
print (find_steps(eg3wire3A, eg3wire3B)) # 410 steps
print ("3) answer part (b):", find_steps(wireA, wireB)) # 5672 steps
