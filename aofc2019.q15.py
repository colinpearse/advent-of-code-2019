...
def advent15a(prog, mapfile, mode='manual', delay=0.1):
    ...
    return "now count the shortest number o steps from '%s' to '%s' manually"%(START,OXYGEN)

def advent15b(mapfile, mode='manual', delay=0.1):
    ...
    while len(set(gmap.values())) != 2:
        ...
        m.msetd(gmap)
        m.printm(j='join', revy=True, message="FILL WITH OXYGEN  delay=%1.3f"%(delay))
...
print ("15) answer part (a):", advent15a(advent15prog, advent15savemap, mode='auto', delay=0.000)) # 270 - counted the steps manually!
print ("15) answer part (b):", advent15b(advent15savemap, mode='auto', delay=0.000)) # 364
